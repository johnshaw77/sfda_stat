...
class LinearRegressionRequest(BaseModel):
    ...
    y: List[float] = Field(..., description="因變數", min_items=2)


class MultipleRegressionRequest(BaseModel):
    """多元迴歸請求模型"""

    x: List[List[float]] = Field(..., description="自變數矩陣", min_items=2)
    y: List[float] = Field(..., description="因變數", min_items=2)
...
class CorrelationRequest(BaseModel):
    ...
    x: List[float] = Field(..., description="變數1", min_items=3)
    y: List[float] = Field(..., description="變數2", min_items=3)
...
class DistributionTestRequest(BaseModel):
    """分佈檢定請求模型"""    values: List[float] = Field(..., description="數值陣列", min_items=8)
